...
The slideshow isn't a dumb shuffle. Each photo rides a "virtual timeline," and freshly added photos carry extra weight that fades over a few days — so when I drop in a new batch, she sees them quickly, then they settle into the rotation instead of dominating it. Add an album from miles away and it simply starts appearing on her wall, no restart required.

It took several attempts at the randomization algorithm to find something that felt random but avoided the parts of true randomness that look wrong on a wall — the same photo twice in an hour, others unseen for weeks. I instrumented the scheduler and actually _measured_ the rate of photo selection to help evaluate different options. I landed on an algorithm where every photo gets a guaranteed minimum spacing before it can come back, while its overall frequency stays untouched. Even coverage without a rigid, predictable order. I described the intent, and the AI wrote the implementation and the tests. I stayed in the part I love.
